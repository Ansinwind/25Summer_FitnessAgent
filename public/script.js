document.addEventListener("DOMContentLoaded", () => {
  ...
  const generatePlanWithAI = async (userProfile, customRequest) => {
    ...
    // 这里我们直接使用模拟数据来展示
    const workoutPart = mockResponse.substring(
      mockResponse.indexOf("<h4>锻炼计划"),
      mockResponse.indexOf("<h4>饮食建议")
    );
    const dietPart = mockResponse.substring(
      mockResponse.indexOf("<h4>饮食建议")
    );
  ...
  const generateRouteWithAI = async () => {
    ...
    if (userRequest === "") {
      routeDescriptionEl.innerHTML =
        '<p style="color:orange;">请输入您的路线需求。</p>';
      return;
    }

    const prompt = `作为一名本地运动向导，请根据以下需求，为我生成一段吸引人的锻炼路线文字描述。需求: "${userRequest}" 请提供路线的大致位置、长度、特点（例如风景、路况、适合时间等）。`;

    const apiKey = "sk-241caae5272d4334a8fd04cf50ee3066";
    const apiUrl =
      "https://dashscope.aliyuncs.com/api/v1/apps/e01812d41c534ed6808e570920feb6a8/completion";
    ...
    try {
      ...
      routeDescriptionEl.innerHTML =
        '<p style="color:red;">路线生成失败，请稍后再试。</p>';
    }
  ...
  const sendMessageToAI = async (userMessage) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `API 请求失败，状态码: ${response.status}. 请检查您的API Key是否正确或已开启。`
        );
      }
      ...
      if (
        ...
        appendMessage(
          "抱歉，我暂时无法生成回复，请检查请求或稍后再试。可能是API返回了空内容。",
          "ai"
        );
      }
    } catch (error) {
  ...
  navButtons.forEach((button, index) => {
    button.addEventListener("click", () => {
      ...
      contentSlider.style.transform = `translateX(-${
        index * (100 / navButtons.length)
      }%)`;
    });
  });
